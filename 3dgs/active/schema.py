...
class BaseSchema:
    init_views: List[int] # Indices of initial views
    load_its: Dict[int, int] # Iteration -> number of views to add at this iteration
...
class VNSeqMInplace(BaseSchema):
    ...
    def __init__(self, dataset_size: int, scene, N: int=20, M: int=1, num_init_views: int=4, interval_epochs=100, uq_reg_holdout_view=False, **kwargs):
        """
        N: int total views to select
        M: # views to select each time
        """
        super().__init__()
        # Start with training view 0
        self.init_views = [0] 
        ...
        # Select initial views by taking the furthest view from the current training set
        ...
        # If using holdout view for uq regressor, select the furthest view from the initial views
        self.holdout_views = []
        if uq_reg_holdout_view:
            trainT = torch.stack([i.camera_center.cpu() for i in train_cams])
            candidateT = torch.stack([i.camera_center.cpu() for i in candidate_cams])
            ...
            holdout_idx = candidate_dist.argmax().item()
            self.holdout_views.append(candidate_views.pop(holdout_idx))
...
class ToNViewsEveryMIterations(BaseSchema):
    """
    Add 1 image every M iterations. (M stays fixed)
    Select up to N images in total.
    """

    def __init__(self, dataset_size: int, scene, N: int=20, M: int=2000, num_init_views: int=1, uq_reg_holdout_view=False, **kwargs):
        """
        N: views to select in total
        M: Iterations between adding new views
        """
        super().__init__()
        # Start with training view 0
        self.init_views = [0] 
        ...
        if num_views_left > 0:
            for i in range(1, num_views_left + 1):
                self.load_its[i * M] = 1

To20Every2000Iterations = partial(ToNViewsEveryMIterations, N=20, M=2000, num_init_views=1)

schema_dict: Dict[str, BaseSchema] = {
                                        'all': All, 
                                        "debug": V20Seq1Debug,
                                        "v20seq1_inplace": V20Seq1Inplace, 
                                        "v10seq1_inplace": V10Seq1Inplace,
                                        "v20seq4_inplace": V20Seq4Inplace,
                                        "to20every2000iterations": To20Every2000Iterations
}
